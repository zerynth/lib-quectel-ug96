...
int _gs_socket_send(int id, uint8_t* buf, int len)
{
    ...
    if (sock->to_be_closed) {
        ...
        if (sock->secure) {
            slot = _gs_acquire_slot(GS_CMD_QSSLSEND, NULL, 32, GS_TIMEOUT * 10, 0);
            _gs_send_at(GS_CMD_QSSLSEND, "=i,i", id, len);
        } else {
            slot = _gs_acquire_slot(GS_CMD_QISEND, NULL, 32, GS_TIMEOUT * 10, 0);
...
int _gs_cell_info(int* mcc, int* mnc)
{
    ...
    if (!slot->err) {
        if (_gs_parse_command_arguments(slot->resp, slot->eresp,
                "SSSii", &s0, &l0, &s1, &l1, &s2, &l2, mcc, mnc)
            != 5) {
            res = 0;
        } else if (s0 && s1 && s2) {
            res = 1;
...
int ug96_gzsock_socket(int family, int type, int protocol) {
    int sock_id = -1;
...
int ug96_gzsock_connect(int sock, const struct sockaddr *addr, socklen_t addrlen) {
    return _gs_socket_connect(sock, (struct sockaddr_in *) addr);
}

int ug96_gzsock_close(int sock) {
    return _gs_socket_close(sock);
}

int ug96_gzsock_send(int sock, const void *dataptr, size_t size, int flags) {
    int32_t wrt;
    int32_t tsnd;
    int32_t err=ERR_OK;
...
int ug96_gzsock_sendto(int sock, const void *dataptr, size_t size, int flags, const struct sockaddr *to, socklen_t tolen) {
    int32_t wrt;
    int32_t tsnd;
    int32_t err=ERR_OK;
    uint8_t *buf = (uint8_t*) dataptr;
...
int ug96_gzsock_recv(int sock, void *mem, size_t len, int flags){
    int rb;
    int trec;
...
int ug96_gzsock_recvfrom(int sock, void *mem, size_t len, int flags, struct sockaddr *from, socklen_t *fromlen) {
    int rb;
    int trec;
    uint8_t *buf = (uint8_t*) mem;
...
int ug96_gzsock_select(int maxfdp1, void *readset, void *writeset, void *exceptset, struct timeval *tv) {
    uint64_t tstart;
    int32_t timeout;
    uint32_t timepast;
...
int ug96_gzsock_read(int sock_id, void *mem, size_t len) {
    return ug96_gzsock_recv(sock_id, mem, len, 0);
}

int ug96_gzsock_write(int sock_id, const void *dataptr, size_t size) {
    return ug96_gzsock_send(sock_id, dataptr, size, 0);
}

int ug96_gzsock_fcntl(int s, int cmd, int val) {
    if (cmd != F_GETFL) {
        return -1;
    }
    return O_NONBLOCK;
...
int ug96_gzsock_shutdown(int s, int how) {
    return 0;
}
...
void ug96_gzsock_freeaddrinfo(struct addrinfo *ai_res) {
    struct addrinfo *p;
...
int ug96_gzsock_setsockopt(int sock_id, int level, int optname, const void *optval, socklen_t optlen) {
...
int ug96_gzsock_getsockopt(int sock_id, int level, int optname, void *optval, socklen_t *optlen) {
    if (optname != SO_RCVTIMEO)
        return -1;
...
int ug96_gzsock_bind(int sock, const struct sockaddr *name, socklen_t namelen){
